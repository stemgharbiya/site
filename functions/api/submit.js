...
export async function onRequestPost(context) {
  try {
    ...
    if (!data.seniorYear || typeof data.seniorYear !== 'string' || !validateSeniorYear(data.seniorYear)) {
      return new Response(JSON.stringify({
    ...
    if (!data.motivation || typeof data.motivation !== 'string' || data.motivation.length > MAX_FIELD_LENGTHS.motivation || data.motivation.trim().length === 0) {
      return new Response(JSON.stringify({
    ...
    for (const interest of interests) {
      if (typeof interest !== 'string' || !ALLOWED_INTERESTS.includes(interest.trim())) {
        return new Response(JSON.stringify({
    ...
    try {
      ...
      const existing = await checkStmt.bind(data.schoolEmail, data.githubUsername).first();
      if (existing) {
        return new Response(JSON.stringify({
    ...
    if (failedEmails.length > 0) {
      console.error('Some emails failed:', failedEmails.map(f => f.reason?.message));
